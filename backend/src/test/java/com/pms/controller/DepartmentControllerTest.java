...
/**
 * Integration tests for the read-only /api/v1/departments endpoints. Departments are provisioned
 * externally, so test data is seeded directly via JDBC rather than through a write endpoint.
 */
...
class DepartmentControllerTest {
    ...
    @Test
    @DisplayName(
            "GET /departments returns active departments by default; includeClosed=true returns all")
    void list_filtersClosed() {
        UUID active = seedDepartment("Active", null, false);
        seedDepartment("Closed", null, true);

        given().when()
                .get("/departments?includeClosed=false")
                .then()
                .statusCode(200)
                .body("size()", equalTo(1))
        ...
        given().when()
                .get("/departments?includeClosed=true")
                .then()
                .statusCode(200)
                .body("size()", equalTo(2));
    }
    ...
    void get_unknown_404() {
        given().when().get("/departments/" + UUID.randomUUID()).then().statusCode(404);
    }
