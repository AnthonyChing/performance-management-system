...
class ManagerEvaluationControllerTest {
    ...
    void setUpAll() {
        ...
        jdbc.execute("UPDATE performance_reviews SET status = 'manager_eval_in_progress', manager_submitted_at = NULL" +
    ...
    void submitEvaluation_whenNotAuthorized_returns403() {
        ...
        given()
                ...
                .body(body)
                .when().patch("/" + NON_SUBORDINATE_USER_ID + "/evaluations/" + EVALUATION_ID)
                .then()
                .statusCode(403)
    ...
    void getEvaluationHistory_forNonSubordinate_returns403() {
        given()
                .when().get("/" + NON_SUBORDINATE_USER_ID + "/evaluations")
                .then()
                .statusCode(403)
                .body("error.code", equalTo("FORBIDDEN"));
    }
    ...
    void submitEvaluation_whenNotInEvalStage_returns409() {
        ...
        given()
                .contentType("application/json")
                .body(body)
                .when().patch("/" + USER_ID + "/evaluations/" + EVALUATION_ID_WRONG_STAGE)
                .then()
                .statusCode(409)
