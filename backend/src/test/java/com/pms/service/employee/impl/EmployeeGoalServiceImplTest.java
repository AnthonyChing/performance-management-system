...
class EmployeeGoalServiceImplTest {
    ...
    void setUp() {
        ...
        activeCycle = PerformanceCycle.builder()
                .id(cycleId)
                .name("2024 Q1")
    ...
    private void mockCurrentCycle() {
        when(performanceCycleRepository.findByStatusIn(anyList()))
                .thenReturn(List.of(activeCycle));
    }

    @Test
    void getGoals_success() {
        mockCurrentCycle();
        Goal goal = Goal.builder().id(UUID.randomUUID()).cycleId(cycleId).ownerId(userId)
                .status(GoalStatus.PENDING_REVIEW).progressPercent(0).title("Test Goal").build();
        ...
        when(goalProgressUpdateRepository.findLatestByGoalIds(anyList())).thenReturn(Collections.emptyList());
        when(goalReviewRepository.findLatestByGoalIds(anyList())).thenReturn(Collections.emptyList());
    ...
    void getHistoricalGoals_withoutCycleId() {
        PerformanceCycle closedCycle = PerformanceCycle.builder().id(UUID.randomUUID()).status(CycleStatus.COMPLETED).build();
        ...
        when(performanceCycleRepository.findByStatusInOrderByHrReviewEndDesc(
                anyList(), any(PageRequest.class))).thenReturn(cyclePage);
        when(goalRepository.findByCycleIdAndOwnerIdAndDeletedAtIsNull(closedCycle.getId(), userId))
                .thenReturn(Collections.emptyList());

        HistoricalGoalsResponseDTO response = goalService.getHistoricalGoals(userId, 1, 10, null, null);
    ...
    void getHistoricalGoals_withCycleId() {
        PerformanceCycle closedCycle = PerformanceCycle.builder().id(cycleId).status(CycleStatus.COMPLETED).build();
        when(performanceCycleRepository.findById(cycleId)).thenReturn(Optional.of(closedCycle));
        
        Goal goal = Goal.builder().id(UUID.randomUUID()).cycleId(cycleId).ownerId(userId)
                .status(GoalStatus.COMPLETED).progressPercent(100).title("Old Goal").build();

        when(goalRepository.findByCycleIdAndOwnerIdAndDeletedAtIsNull(cycleId, userId))
                .thenReturn(List.of(goal));

        HistoricalGoalsResponseDTO response = goalService.getHistoricalGoals(userId, 1, 10, "old", cycleId.toString());
    ...
    void updateGoal_success() {
        ...
        Goal goal = Goal.builder().id(goalId).cycleId(cycleId).ownerId(userId)
                .status(GoalStatus.REVISION_REQUESTED).build();
    ...
    void updateGoalProgress_success() {
        ...
        GoalProgressUpdate update = GoalProgressUpdate.builder().id(UUID.randomUUID()).progressPercent(50).updatedBy(userId).build();
        ...
        GoalProgressUpdateResponseDTO response = goalService.updateGoalProgress(userId, goalId.toString(), req);
    ...
    void getGoalReviewResult_success() {
        ...
        GoalReview review = GoalReview.builder().id(UUID.randomUUID()).goalId(goal.getId())
                .decision(GoalReviewDecision.APPROVED).reviewedBy(managerId).reviewedAt(OffsetDateTime.now()).build();
        ...
        assertEquals(GoalReviewDecision.APPROVED.getDbValue(), response.getResults().get(0).getDecision());
    }

    @Test
    void getGoals_withSearchQuery() {
        mockCurrentCycle();
        Goal goal1 = Goal.builder().id(UUID.randomUUID()).cycleId(cycleId).ownerId(userId)
                .status(GoalStatus.PENDING_REVIEW).title("Find this target").build();
        Goal goal2 = Goal.builder().id(UUID.randomUUID()).cycleId(cycleId).ownerId(userId)
                .status(GoalStatus.PENDING_REVIEW).title("Ignore this").build();
    ...
    void getGoals_invalidStatus() {
        mockCurrentCycle();
        assertThrows(ConflictException.class, () -> goalService.getGoals(userId, "invalid_status_string", null));
    }

    @Test
    void createGoal_lockedCycle() {
        PerformanceCycle lockedCycle = PerformanceCycle.builder()
                .id(cycleId)
                .status(CycleStatus.IN_PROGRESS)
                .isLocked(true)
