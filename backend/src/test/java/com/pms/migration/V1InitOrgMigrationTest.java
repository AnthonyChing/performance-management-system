...
/**
 * Verifies that Flyway migration V1__init_org.sql applies cleanly against a real
 * PostgreSQL container and produces the schema described in
 * docs/database/new-schema.md (Identity & Org section).
 *
 * <p>Spring Boot runs Flyway during context startup, so reaching the assertions
 * already proves the migration executed without error. Each test then sanity-checks
 * a specific aspect: tables present, enums present, seed roles, and the partial
 * unique index that guards "at most one open department membership per user".
 */
@SpringBootTest
@ActiveProfiles("test")
...
class V1InitOrgMigrationTest {
    ...
    @Test
    @DisplayName("Enums employment_status_enum and identity_provider_enum exist with expected values")
    void enums_haveExpectedValues() {
        List<String> employmentStatuses = jdbc.queryForList(
        ...
        assertThat(providers)
                .containsExactlyInAnyOrder("google", "azure_ad", "okta", "local");
    }

    @Test
    @DisplayName("Seed inserts the four canonical roles")
    void roles_areSeeded() {
        Set<String> names = Set.copyOf(jdbc.queryForList(
    ...
    void departments_treeStructure() {
        ...
        jdbc.update("INSERT INTO departments (id, name, parent_id) VALUES (?::uuid, 'Child', ?::uuid)",
                childId, parentId);

        String resolvedParent = jdbc.queryForObject(
