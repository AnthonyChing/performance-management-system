...
public interface PerformanceCycleRepository extends JpaRepository<PerformanceCycle, UUID> {
    ...
    @Query(
            value = "SELECT * FROM performance_cycles WHERE status::text IN (:statuses)",
            nativeQuery = true)
    List<PerformanceCycle> findByStatusInStrings(@Param("statuses") Collection<String> statuses);

    @Query(
            value =
                    "SELECT * FROM performance_cycles WHERE status::text IN (:statuses) ORDER BY hr_review_end DESC",
            countQuery =
                    "SELECT count(*) FROM performance_cycles WHERE status::text IN (:statuses)",
            nativeQuery = true)
    Page<PerformanceCycle> findByStatusInStringsOrderByHrReviewEndDesc(
    ...
    default Page<PerformanceCycle> findByStatusInOrderByHrReviewEndDesc(
            List<CycleStatus> statuses, Pageable pageable) {
        return findByStatusInStringsOrderByHrReviewEndDesc(
                statuses.stream().map(CycleStatus::getDbValue).toList(), pageable);
    }
