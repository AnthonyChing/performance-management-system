...
public class HrCycleController {
    ...
    public ResponseEntity<CycleResponseDTO> createCycle(
            @Valid @RequestBody CycleCreateRequestDTO req) {
        CycleResponseDTO created = cycleService.createCycle(SecurityUtils.currentUserId(), req);
        return ResponseEntity.created(
                        URI.create("/api/v1/hr/performance-cycles/" + created.getId()))
                .body(created);
    }
    ...
    public ResponseEntity<CycleResponseDTO> patchCycle(
            @PathVariable UUID cycleId, @RequestBody CyclePatchRequestDTO req) {
        return ResponseEntity.ok(
                cycleService.patchCycle(SecurityUtils.currentUserId(), cycleId, req));
    }

    @PatchMapping("/{cycleId}/status")
    public ResponseEntity<CycleResponseDTO> patchCycleStatus(
            @PathVariable UUID cycleId, @Valid @RequestBody CyclePatchStatusRequestDTO req) {
