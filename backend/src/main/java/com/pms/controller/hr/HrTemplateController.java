...
public class HrTemplateController {
    ...
    public ResponseEntity<TemplateResponseDTO> createTemplate(
        ...
        TemplateResponseDTO created = templateService.createTemplate(SecurityUtils.currentUserId(), req);
        return ResponseEntity.created(URI.create("/api/v1/hr/assessment-templates/" + created.getId())).body(created);
    }
    ...
    public ResponseEntity<Map<String, Object>> listTemplates(
        ...
        Page<TemplateListItemDTO> result = templateService.listTemplates(status, jobFunction, page, pageSize);
    ...
    public ResponseEntity<TemplateResponseDTO> patchTemplate(
            ...
            @RequestBody TemplatePatchRequestDTO req) {
        return ResponseEntity.ok(templateService.patchTemplate(SecurityUtils.currentUserId(), templateId, req));
    }
    ...
    public ResponseEntity<TemplateResponseDTO> duplicateTemplate(@PathVariable UUID templateId) {
        TemplateResponseDTO copy = templateService.duplicateTemplate(SecurityUtils.currentUserId(), templateId);
    ...
    public ResponseEntity<Map<String, String>> applyTemplate(
            @PathVariable UUID templateId,
            @RequestBody TemplateApplicationRequestDTO req) {
        templateService.applyTemplate(templateId, req);
        return ResponseEntity.ok(Map.of("message", "Template applied successfully to selected groups."));
    }
    ...
    public ResponseEntity<QuestionResponseDTO> addQuestion(
            @PathVariable UUID templateId,
            @Valid @RequestBody QuestionCreateRequestDTO req) {
        QuestionResponseDTO created = templateService.addQuestion(SecurityUtils.currentUserId(), templateId, req);
        return ResponseEntity.status(201).body(created);
    }

    @GetMapping("/{templateId}/questions")
    public ResponseEntity<Map<String, List<QuestionResponseDTO>>> listQuestions(@PathVariable UUID templateId) {
    ...
    public ResponseEntity<QuestionResponseDTO> patchQuestion(
        ...
        return ResponseEntity.ok(templateService.patchQuestion(SecurityUtils.currentUserId(), templateId, questionId, req));
    }
    ...
    public ResponseEntity<Map<String, String>> reorderQuestions(
            @PathVariable UUID templateId,
            @Valid @RequestBody QuestionReorderRequestDTO req) {
