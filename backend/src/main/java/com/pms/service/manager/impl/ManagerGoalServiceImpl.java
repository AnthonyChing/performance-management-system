...
public class ManagerGoalServiceImpl implements ManagerGoalService {
    ...
    public ManagerGoalResponseDTO patchGoal(UUID managerId, UUID subordinateId, UUID goalId, ManagerGoalPatchRequestDTO req) {
        ...
        Goal goal = goalRepo.findByIdAndDeletedAtIsNull(goalId)
                ...
                .orElseThrow(() -> new NotFoundException("RESOURCE_NOT_FOUND", "Goal not found"));
        PerformanceCycle cycle = cycleRepo.findById(goal.getCycleId())
    ...
    @Override
    public List<ManagerGoalResponseDTO> listGoals(UUID managerId, UUID subordinateId, String cycleId, String status) {
        assertDirectSubordinate(managerId, subordinateId);
        List<Goal> goals;
        if (cycleId != null) {
            ...
            PerformanceCycle cycle = getCurrentCycle();
            goals = goalRepo.findByCycleIdAndOwnerIdAndDeletedAtIsNull(cycle.getId(), subordinateId);
        }
        if (status != null) {
    ...
    private GoalType parseGoalType(String value) {
        try { return GoalType.fromDbValue(value); }
            ...
            throw new ApiException(HttpStatus.BAD_REQUEST, "VALIDATION_ERROR", "Invalid goal_type: " + value);
        }
    }

    private GoalStatus parseGoalStatus(String value) {
        try { return GoalStatus.fromDbValue(value); }
        catch (IllegalArgumentException e) {
