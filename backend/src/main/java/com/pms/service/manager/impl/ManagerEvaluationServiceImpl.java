...
public class ManagerEvaluationServiceImpl implements ManagerEvaluationService {
    ...
    private static final java.util.Set<ReviewStatus> MANAGER_EVAL_STATUSES = java.util.Set.of(
    ...
    public ManagerQuestionnaireResponseDTO updateQuestionnaire(
        ...
        List<ReviewResponse> saved = reviewResponseRepo
                .findByReviewIdAndRespondentTypeOrderByRespondedAtAsc(evaluationId, RESPONDENT_TYPE);
    ...
    public ManagerEvaluationResponseDTO updateKpiEvaluation(
        ...
        if (req.getKpiEvaluations() != null) {
            for (KpiEvaluationItemDTO item : req.getKpiEvaluations()) {
                KpiEvaluation eval = kpiEvalRepo.findByReviewIdAndKpiId(evaluationId, item.getKpiId())
                        .orElse(KpiEvaluation.builder()
                                .id(UUID.randomUUID())
                                .reviewId(evaluationId)
                                .kpiId(item.getKpiId())
    ...
    @Override
    public List<ManagerEvaluationResponseDTO> listEvaluations(UUID managerId, UUID subordinateId, String cycleId) {
        assertDirectSubordinate(managerId, subordinateId);
        List<PerformanceReview> reviews = reviewRepo.findByEmployeeId(subordinateId).stream()
                .filter(r -> r.getManagerId().equals(managerId) || r.getEmployeeId().equals(subordinateId))
                .toList();
        if (cycleId != null) {
            UUID cid = UUID.fromString(cycleId);
            reviews = reviews.stream().filter(r -> r.getCycleId().equals(cid)).toList();
        }
        return reviews.stream().map(r -> ManagerEvaluationResponseDTO.from(
                r,
                reviewResponseRepo.findByReviewIdAndRespondentTypeOrderByRespondedAtAsc(r.getId(), RESPONDENT_TYPE),
                kpiEvalRepo.findByReviewId(r.getId()))).toList();
    }

    private PerformanceReview getReviewAndValidate(UUID managerId, UUID subordinateId, UUID evaluationId) {
        assertDirectSubordinate(managerId, subordinateId);
        PerformanceReview review = reviewRepo.findById(evaluationId)
                .orElseThrow(() -> new NotFoundException("RESOURCE_NOT_FOUND", "Evaluation not found"));
        if (!review.getEmployeeId().equals(subordinateId)) {
            throw new ForbiddenException("FORBIDDEN", "This evaluation does not belong to the specified employee");
        }
        if (!review.getManagerId().equals(managerId)) {
            throw new ForbiddenException("FORBIDDEN", "You are not the assigned manager for this evaluation");
        }
        return review;
    }
    ...
    private ReviewStatus parseReviewStatus(String value) {
        try { return ReviewStatus.fromDbValue(value); }
    ...
    private RatingScale parseRatingScale(String value) {
        try { return RatingScale.fromDbValue(value); }
        catch (IllegalArgumentException e) {
            throw new ApiException(HttpStatus.BAD_REQUEST, "VALIDATION_ERROR", "Invalid final_rating: " + value);
        }
    }
}
