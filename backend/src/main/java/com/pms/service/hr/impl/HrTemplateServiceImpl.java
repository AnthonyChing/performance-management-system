...
public class HrTemplateServiceImpl implements HrTemplateService {
    ...
    @Override
    public Page<TemplateListItemDTO> listTemplates(String status, String jobFunction, int page, int pageSize) {
        return templateRepo.findAllFiltered(status, jobFunction, PageRequest.of(page - 1, pageSize))
    ...
    public TemplateResponseDTO patchTemplate(UUID actorId, UUID templateId, TemplatePatchRequestDTO req) {
        ...
        if (t.getStatus() == TemplateStatus.PUBLISHED && templateRepo.countCycleUsages(templateId) > 0) {
            throw new ConflictException("STATE_CONFLICT", "Cannot modify a template that is in use by active cycles");
        }
    ...
    public void deleteTemplate(UUID templateId) {
        ...
        if (templateRepo.countCycleUsages(templateId) > 0) {
            throw new ConflictException("STATE_CONFLICT", "Cannot delete a template that is referenced by performance cycles");
        }
        t.setIsActive(false);
    ...
    @Override
    @Transactional
    public QuestionResponseDTO addQuestion(UUID actorId, UUID templateId, QuestionCreateRequestDTO req) {
        AssessmentTemplate t = findActiveTemplate(templateId);
        if (t.getStatus() == TemplateStatus.PUBLISHED) {
            throw new ConflictException("STATE_CONFLICT", "Cannot add questions to a published template");
        }
        ...
        TemplateQuestion q = TemplateQuestion.builder()
                .id(UUID.randomUUID())
                .templateId(templateId)
    ...
    @Override
    @Transactional
    public QuestionResponseDTO patchQuestion(UUID actorId, UUID templateId, UUID questionId, QuestionPatchRequestDTO req) {
        AssessmentTemplate t = findActiveTemplate(templateId);
        if (t.getStatus() == TemplateStatus.PUBLISHED && templateRepo.countCycleUsages(templateId) > 0) {
            throw new ConflictException("STATE_CONFLICT", "Cannot modify questions of a template in use");
        }
        TemplateQuestion q = questionRepo.findByIdAndTemplateIdAndDeletedAtIsNull(questionId, templateId)
                .orElseThrow(() -> new NotFoundException("RESOURCE_NOT_FOUND", "Question not found"));
        if (req.getQuestionText() != null) q.setQuestionText(req.getQuestionText());
        if (req.getQuestionType() != null) q.setQuestionType(QuestionType.fromDbValue(req.getQuestionType()));
    ...
    public List<QuestionResponseDTO> listQuestions(UUID templateId) {
        findActiveTemplate(templateId);
        return questionRepo.findByTemplateIdAndDeletedAtIsNullOrderBySortOrderAsc(templateId)
                .stream().map(QuestionResponseDTO::from).toList();
    }
    ...
    public void reorderQuestions(UUID templateId, QuestionReorderRequestDTO req) {
        AssessmentTemplate t = findActiveTemplate(templateId);
        if (t.getStatus() == TemplateStatus.PUBLISHED) {
            throw new ConflictException("STATE_CONFLICT", "Cannot reorder questions of a published template");
        }
        List<TemplateQuestion> existing = questionRepo
                .findByTemplateIdAndDeletedAtIsNullOrderBySortOrderAsc(templateId);
        ...
        for (int i = 0; i < req.getOrderedQuestionIds().size(); i++) {
            ...
            TemplateQuestion q = existing.stream()
                    .filter(x -> x.getId().equals(qId)).findFirst()
    ...
    private AssessmentTemplate findActiveTemplate(UUID templateId) {
        return templateRepo.findByIdAndDeletedAtIsNull(templateId)
                .orElseThrow(() -> new NotFoundException("RESOURCE_NOT_FOUND", "Template not found"));
    }

    private void rebalanceSortOrder(UUID templateId) {
        List<TemplateQuestion> remaining = questionRepo
                .findByTemplateIdAndDeletedAtIsNullOrderBySortOrderAsc(templateId);
