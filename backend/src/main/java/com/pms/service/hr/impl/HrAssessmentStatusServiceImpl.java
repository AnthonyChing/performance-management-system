...
public class HrAssessmentStatusServiceImpl implements HrAssessmentStatusService {
    ...
    public Page<AssessmentStatusDTO> listAssessmentStatuses(
        ...
        List<UUID> employeeIds =
                reviews.stream().map(PerformanceReview::getEmployeeId).distinct().toList();
        ...
        List<UUID> deptIds =
                userMap.values().stream().map(User::getDepartmentId).distinct().toList();
        ...
        return reviews.map(
                review -> {
                    ...
                    Department dept =
                            employee != null ? deptMap.get(employee.getDepartmentId()) : null;
