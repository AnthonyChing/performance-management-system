...
public class EmployeeKpiServiceImpl implements EmployeeKpiService {
    ...
    public KpiStandardsResponseDTO getKpiStandards(UUID userId) {
        PerformanceCycle cycle = getCurrentCycleOptional()
                .orElseThrow(() -> new NotFoundException("CURRENT_KPI_CYCLE_NOT_FOUND", "No current KPI cycle found"));

        PerformanceReview review = performanceReviewRepository.findByCycleIdAndEmployeeId(cycle.getId(), userId)
                .orElseThrow(() -> new NotFoundException("KPI_REVIEW_NOT_FOUND", "No performance review found for current cycle"));
        ...
        List<KpiAssignment> assignments = kpiAssignmentRepository.findByUserIdAndKpiIdIn(userId, kpiIds);

        Map<UUID, KpiAssignment> assignmentMap = assignments.stream()
                .collect(Collectors.toMap(KpiAssignment::getKpiId, Function.identity()));

        List<KpiStandardDTO> standards = kpis.stream()
                ...
                .map(kpi -> {
                    ...
                    return KpiStandardDTO.builder()
                            ...
                            .weightPercent(assignment.getWeight() != null
                                    ? assignment.getWeight().intValue()
                                    : 0)
                            .target(KpiStandardDTO.KpiTargetDTO.builder()
                                    .operator(kpi.getTargetOperator())
                                    .value(kpi.getTargetDisplayText() != null
                                            ? (assignment.getTargetValue() != null ? assignment.getTargetValue().doubleValue() : null)
                                            : (kpi.getTargetValue() != null ? kpi.getTargetValue().doubleValue() : null))
    ...
    public KpiResultResponseDTO getKpiResult(UUID userId) {
        ...
        if ("not_published".equals(resultStatus)) {
            return KpiResultResponseDTO.builder()
                    .result(KpiResultSummaryDTO.builder()
                            ...
                            .availableActions(AvailableActionsDTO.builder()
                                    .canConfirm(false)
                                    .confirmUnavailableReason("result_not_published")
                                    .canDispute(false)
                                    .disputeUnavailableReason("result_not_published")
                                    .build())
                            .build())
                    .build();
        }
        ...
        Map<UUID, KpiAssignment> assignmentMap = assignments.stream()
                .collect(Collectors.toMap(KpiAssignment::getKpiId, Function.identity()));
        List<Kpi> assignedKpis = kpis.stream()
        ...
        for (Kpi kpi : assignedKpis) {
            KpiAssignment assignment = assignmentMap.get(kpi.getId());
            double targetVal = assignment.getTargetValue() != null ? assignment.getTargetValue().doubleValue() : 1.0;
            double currentVal = assignment.getCurrentValue() != null ? assignment.getCurrentValue().doubleValue() : 0.0;
            double weight = assignment.getWeight() != null ? assignment.getWeight().doubleValue() : 0.0;
            ...
            Optional<KpiProgressSnapshot> snapshotOpt =
                    kpiProgressSnapshotRepository.findTopByKpiIdAndUserIdOrderByRecordedAtDesc(kpi.getId(), userId);
            KpiResultDTO.KpiSnapshotDTO snapshotDTO = snapshotOpt.map(s -> KpiResultDTO.KpiSnapshotDTO.builder()
                    .snapshotId(String.valueOf(s.getId()))
                    .value(s.getValue() != null ? s.getValue().doubleValue() : null)
                    .note(s.getNote())
                    .recordedAt(s.getRecordedAt())
                    .build()).orElse(null);

            kpiResults.add(KpiResultDTO.builder()
                    .kpiId(kpi.getId().toString())
                    .name(kpi.getTitle())
                    .weightPercent((int) weight)
                    .actual(KpiResultDTO.KpiActualDTO.builder()
                            .value(currentVal)
                            .unit(kpi.getUnit())
        ...
        Optional<KpiResultConfirmation> confirmationOpt = kpiResultConfirmationRepository.findByReviewId(review.getId());
        ...
        KpiResultSummaryDTO.DisputePeriodDTO disputePeriod = KpiResultSummaryDTO.DisputePeriodDTO.builder()
        ...
        boolean canDispute = "pending_confirmation".equals(resultStatus) && "open".equals(disputeStatus);
        ...
        if (confirmationOpt.isPresent()) {
            ...
            confirmationDTO = KpiResultSummaryDTO.KpiConfirmationDTO.builder()
                    .confirmationId(conf.getId().toString())
                    .resultId(review.getId().toString())
                    .confirmedAt(conf.getConfirmedAt())
                    .confirmedBy(buildEmployeeSummaryDTO(userRepository.findById(conf.getConfirmedBy()).orElse(null)))
                    .build();
        }
        ...
        return KpiResultResponseDTO.builder()
                .result(KpiResultSummaryDTO.builder()
                        ...
                        .availableActions(AvailableActionsDTO.builder()
                                ...
                                .disputeUnavailableReason(canDispute ? null : disputeStatus.equals("open") ? null : disputeStatus)
                                .build())
    ...
    public KpiConfirmationResponseDTO confirmKpiResult(UUID userId, KpiConfirmationRequestDTO request) {
        ...
        PerformanceReview review = performanceReviewRepository.findById(reviewId)
                .orElseThrow(() -> new NotFoundException("KPI_RESULT_NOT_FOUND", "KPI result (review) not found"));

        if (!review.getEmployeeId().equals(userId)) {
            throw new ForbiddenException("FORBIDDEN", "You do not have permission to confirm this KPI result");
        }

        PerformanceCycle cycle = performanceCycleRepository.findById(review.getCycleId())
                .orElseThrow(() -> new NotFoundException("CURRENT_KPI_CYCLE_NOT_FOUND", "Cycle not found"));

        String resultStatus = computeKpiResultStatus(cycle, review, userId);
        if (!"pending_confirmation".equals(resultStatus)) {
            if ("confirmed".equals(resultStatus)) {
                throw new ConflictException("KPI_RESULT_ALREADY_CONFIRMED", "KPI result has already been confirmed");
            } else if ("disputed".equals(resultStatus)) {
                throw new ConflictException("KPI_RESULT_ALREADY_DISPUTED", "KPI result has already been disputed");
            } else {
                throw new ConflictException("INVALID_KPI_RESULT_STATUS", "KPI result cannot be confirmed in current status: " + resultStatus);
            }
        }
        ...
        KpiResultConfirmation confirmation = KpiResultConfirmation.builder()
                .id(UUID.randomUUID())
                .reviewId(reviewId)
                .confirmedBy(userId)
        ...
        User user = userRepository.findById(userId).orElse(null);
        KpiResultSummaryDTO.KpiConfirmationDTO confirmationDTO = KpiResultSummaryDTO.KpiConfirmationDTO.builder()
    ...
    @Override
    public HistoricalKpiResultsResponseDTO getHistoricalKpiResults(UUID userId, Integer page, Integer pageSize, String q, String cycleId) {
        if (cycleId == null) {
            List<PerformanceReview> reviews = performanceReviewRepository.findByEmployeeId(userId);
            Set<UUID> reviewCycleIds = reviews.stream().map(PerformanceReview::getCycleId).collect(Collectors.toSet());

            List<CycleStatus> historicalStatuses = List.of(CycleStatus.COMPLETED, CycleStatus.CLOSED);
            List<PerformanceCycle> historicalCycles = performanceCycleRepository.findByStatusIn(historicalStatuses)
                    .stream()
                    .filter(c -> reviewCycleIds.contains(c.getId()))
                    .sorted(Comparator.comparing(PerformanceCycle::getHrReviewEnd).reversed())
                    .collect(Collectors.toList());

            // apply q filter on cycle name
            if (q != null && !q.isBlank()) {
                String lq = q.toLowerCase();
                historicalCycles = historicalCycles.stream()
            ...
            Map<UUID, PerformanceReview> reviewByCycleId = reviews.stream()
                    .collect(Collectors.toMap(PerformanceReview::getCycleId, Function.identity(), (a, b) -> a));

            List<KpiResultSummaryDTO> results = pagedCycles.stream().map(c -> {
                ...
                return KpiResultSummaryDTO.builder()
                        ...
                        .employee(buildEmployeeSummaryDTO(userRepository.findById(userId).orElse(null)))
                        .status(r != null ? r.getStatus().getDbValue() : null)
                        .build();
            }).collect(Collectors.toList());
            ...
            PerformanceReview review = performanceReviewRepository.findByCycleIdAndEmployeeId(cycle.getId(), userId)
                    .orElseThrow(() -> new NotFoundException("KPI_REVIEW_NOT_FOUND", "No KPI review found for cycle"));
            ...
            List<KpiStandardDTO> standards = kpis.stream()
                    ...
                    .map(k -> {
                        ...
                        return KpiStandardDTO.builder()
                                ...
                                .weightPercent(a.getWeight() != null ? a.getWeight().intValue() : 0)
                                .target(KpiStandardDTO.KpiTargetDTO.builder()
                                        .operator(k.getTargetOperator())
                                        .value(a.getTargetValue() != null ? a.getTargetValue().doubleValue() : null)
                                        .unit(k.getTargetUnit())
                                        .displayText(k.getTargetDisplayText())
                                        .build())
                                .build();
                    }).collect(Collectors.toList());

            User user = userRepository.findById(userId).orElse(null);
            KpiResultSummaryDTO result = KpiResultSummaryDTO.builder()
                    .resultId(review.getId().toString())
                    .cycle(buildCycleSummaryDTO(cycle))
                    .employee(buildEmployeeSummaryDTO(user))
    ...
    private String computeKpiResultStatus(PerformanceCycle cycle, PerformanceReview review, UUID userId) {
        if (cycle.getResultsPublishedAt() == null) return "not_published";
        if (kpiResultConfirmationRepository.findByReviewId(review.getId()).isPresent()) return "confirmed";
        if (appealRepository.findByReviewId(review.getId()).isPresent()) return "disputed";
        if (cycle.getStatus() == CycleStatus.COMPLETED || cycle.getStatus() == CycleStatus.CLOSED) return "finalized";
