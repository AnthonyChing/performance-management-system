...
import { HISTORY_GOALS_MOCK } from '../../../shared/api/mockData';
          ...
          <div key={item.id} className="bg-white rounded-lg border border-slate-200 shadow-sm p-6 flex flex-col md:flex-row md:items-center justify-between hover:shadow-md transition-shadow">
            <div className="flex-1 mb-4 md:mb-0">
              ...
              <p className="text-xs text-slate-500 mt-1 flex items-center">
                 <span className="text-slate-400 mr-2">考核區間：</span>{item.dates}
              </p>
            </div>
            ...
            <div className="flex items-center space-x-12">
              <div className="text-center w-24">
                ...
                <p className="text-2xl font-bold text-slate-800">{item.avgCompletion}%</p>
              </div>
              ...
              <Link to={`/goals/history/${item.id}`} className="flex items-center text-sm font-medium text-indigo-600 hover:text-indigo-800 ml-4 group min-w-[80px] justify-end">
                查看詳情
